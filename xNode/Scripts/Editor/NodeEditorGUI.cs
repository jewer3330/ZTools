...
namespace XNodeEditor {
    ...
    public partial class NodeEditorWindow {
        ...
        private List<ZTool.XNode.Node> culledNodes;
        ...
        void ShowPortContextMenu(ZTool.XNode.NodePort hoveredPort) {
            GenericMenu contextMenu = new GenericMenu();
            ...
            if (NodeEditorPreferences.GetSettings().createFilter) {
                ...
                if (hoveredPort.direction == ZTool.XNode.NodePort.IO.Input)
                    graphEditor.AddContextMenuItems(contextMenu, hoveredPort.ValueType, ZTool.XNode.NodePort.IO.Output);
                else
                    graphEditor.AddContextMenuItems(contextMenu, hoveredPort.ValueType, ZTool.XNode.NodePort.IO.Input);
            }
            contextMenu.DropDown(new Rect(Event.current.mousePosition, Vector2.zero));
            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
        }
            ...
            foreach (ZTool.XNode.Node node in graph.nodes) {
                ...
                foreach (ZTool.XNode.NodePort output in node.Outputs) {
                    ...
                    for (int k = 0; k < output.ConnectionCount; k++) {
                        ZTool.XNode.NodePort input = output.GetConnection(k);
        ...
        private void DrawNodes() {
            ...
            if (Selection.activeObject != null && Selection.activeObject is ZTool.XNode.Node) {
            ...
            List<ZTool.XNode.NodePort> removeEntries = new List<ZTool.XNode.NodePort>();

            if (e.type == EventType.Layout) culledNodes = new List<ZTool.XNode.Node>();
            for (int n = 0; n < graph.nodes.Count; n++) {
                // Skip null nodes. The user could be in the process of renaming scripts, so removing them at this point is not advisable.
                if (graph.nodes[n] == null) continue;
                if (n >= graph.nodes.Count) return;
                ZTool.XNode.Node node = graph.nodes[n];
        ...
        private bool ShouldBeCulled(ZTool.XNode.Node node) {
