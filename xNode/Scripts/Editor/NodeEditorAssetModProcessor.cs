...
namespace XNodeEditor {
    ...
    class NodeEditorAssetModProcessor : UnityEditor.AssetModificationProcessor {
        ...
        private static AssetDeleteResult OnWillDeleteAsset (string path, RemoveAssetOptions options) {
            ...
            if (scriptType == null || (scriptType != typeof (ZTool.XNode.Node) && !scriptType.IsSubclassOf (typeof (ZTool.XNode.Node)))) return AssetDeleteResult.DidNotDelete;
            ...
            for (int i = 0; i < guids.Length; i++) {
                ...
                for (int k = 0; k < objs.Length; k++) {
                    ZTool.XNode.Node node = objs[k] as ZTool.XNode.Node;
        ...
        private static void OnReloadEditor () {
            ...
            string[] guids = AssetDatabase.FindAssets ("t:" + typeof (ZTool.XNode.NodeGraph));
            for (int i = 0; i < guids.Length; i++) {
                string assetpath = AssetDatabase.GUIDToAssetPath (guids[i]);
                ZTool.XNode.NodeGraph graph = AssetDatabase.LoadAssetAtPath (assetpath, typeof (ZTool.XNode.NodeGraph)) as ZTool.XNode.NodeGraph;
                ...
                for (int u = 0; u < objs.Length; u++) {
                    ...
                    if (!graph.nodes.Contains (objs[u] as ZTool.XNode.Node)) graph.nodes.Add(objs[u] as ZTool.XNode.Node);
                }
            }
        }
    }
