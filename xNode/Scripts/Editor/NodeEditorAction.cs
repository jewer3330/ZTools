...
namespace XNodeEditor {
    public partial class NodeEditorWindow {
        ...
        public static ZTool.XNode.Node[] copyBuffer = null;
        ...
        public ZTool.XNode.NodePort DraggedOutputPort { get { ZTool.XNode.NodePort result = draggedOutput; return result; } }
        /// <summary> Return the Hovered port or null if not exist </summary>
        public ZTool.XNode.NodePort HoveredPort { get { ZTool.XNode.NodePort result = hoveredPort; return result; } }
        /// <summary> Return the Hovered node or null if not exist </summary>
        public ZTool.XNode.Node HoveredNode { get { ZTool.XNode.Node result = hoveredNode; return result; } }

        private ZTool.XNode.Node hoveredNode = null;
        [NonSerialized] public ZTool.XNode.NodePort hoveredPort = null;
        [NonSerialized] private ZTool.XNode.NodePort draggedOutput = null;
        [NonSerialized] private ZTool.XNode.NodePort draggedOutputTarget = null;
        [NonSerialized] private ZTool.XNode.NodePort autoConnectOutput = null;
            ...
            switch (e.type) {
                    ...
                    if (e.button == 0) {
                        ...
                        if (currentActivity == NodeActivity.DragNode) {
                            ...
                            for (int i = 0; i < Selection.objects.Length; i++) {
                                if (Selection.objects[i] is ZTool.XNode.Node) {
                                    ZTool.XNode.Node node = Selection.objects[i] as ZTool.XNode.Node;
                                    ...
                                    if (offset.sqrMagnitude > 0) {
                                        foreach (ZTool.XNode.NodePort output in node.Outputs) {
                                            Rect rect;
                                        ...
                                        foreach (ZTool.XNode.NodePort input in node.Inputs) {
                                            Rect rect;
                        ...
                        if (IsHoveringPort) {
                            if (hoveredPort.IsOutput) {
                                ...
                                if (hoveredPort.IsConnected) {
                                    ZTool.XNode.Node node = hoveredPort.node;
                                    ZTool.XNode.NodePort output = hoveredPort.Connection;
                    ...
                    if (e.button == 0) {
                        ...
                        if (IsDraggingPort) {
                            ...
                            if (draggedOutputTarget != null && draggedOutput.CanConnectTo(draggedOutputTarget)) {
                                ZTool.XNode.Node node = draggedOutputTarget.node;
                            ...
                            IEnumerable<ZTool.XNode.Node> nodes = Selection.objects.Where(x => x is ZTool.XNode.Node).Select(x => x as ZTool.XNode.Node);
                            foreach (ZTool.XNode.Node node in nodes) EditorUtility.SetDirty(node);
                            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
                        } else if (!IsHoveringNode) {
                ...
                case EventType.KeyDown:
                    ...
                    if (e.keyCode == KeyCode.A) {
                        if (Selection.objects.Any(x => graph.nodes.Contains(x as ZTool.XNode.Node))) {
                            foreach (ZTool.XNode.Node node in graph.nodes) {
                                DeselectNode(node);
                            }
                        } else {
                            foreach (ZTool.XNode.Node node in graph.nodes) {
        ...
        public void Home() {
            var nodes = Selection.objects.Where(o => o is ZTool.XNode.Node).Cast<ZTool.XNode.Node>().ToList();
            if (nodes.Count > 0) {
        ...
        public void RemoveSelectedNodes() {
            ...
            foreach (UnityEngine.Object item in Selection.objects) {
                if (item is ZTool.XNode.Node) {
                    ZTool.XNode.Node node = item as ZTool.XNode.Node;
        ...
        public void RenameSelectedNode() {
            if (Selection.objects.Length == 1 && Selection.activeObject is ZTool.XNode.Node) {
                ZTool.XNode.Node node = Selection.activeObject as ZTool.XNode.Node;
                Vector2 size;
        ...
        public void MoveNodeToTop(ZTool.XNode.Node node) {
            int index;
        ...
        public void DuplicateSelectedNodes() {
            ...
            ZTool.XNode.Node[] selectedNodes = Selection.objects.Select(x => x as ZTool.XNode.Node).Where(x => x != null && x.graph == graph).ToArray();
        ...
        public void CopySelectedNodes() {
            copyBuffer = Selection.objects.Select(x => x as ZTool.XNode.Node).Where(x => x != null && x.graph == graph).ToArray();
        }
        ...
        private void InsertDuplicateNodes(ZTool.XNode.Node[] nodes, Vector2 topLeft) {
            ...
            Dictionary<ZTool.XNode.Node, ZTool.XNode.Node> substitutes = new Dictionary<ZTool.XNode.Node, ZTool.XNode.Node>();
            for (int i = 0; i < nodes.Length; i++) {
                ZTool.XNode.Node srcNode = nodes[i];
                if (srcNode == null) continue;

                // Check if user is allowed to add more of given node type
                ZTool.XNode.Node.DisallowMultipleNodesAttribute disallowAttrib;
                ...
                ZTool.XNode.Node newNode = graphEditor.CopyNode(srcNode);
            ...
            for (int i = 0; i < nodes.Length; i++) {
                ZTool.XNode.Node srcNode = nodes[i];
                if (srcNode == null) continue;
                foreach (ZTool.XNode.NodePort port in srcNode.Ports) {
                    for (int c = 0; c < port.ConnectionCount; c++) {
                        ZTool.XNode.NodePort inputPort = port.direction == ZTool.XNode.NodePort.IO.Input ? port : port.GetConnection(c);
                        ZTool.XNode.NodePort outputPort = port.direction == ZTool.XNode.NodePort.IO.Output ? port : port.GetConnection(c);

                        ZTool.XNode.Node newNodeIn, newNodeOut;
        ...
        bool IsHoveringTitle(ZTool.XNode.Node node) {
        ...
        public void AutoConnect(ZTool.XNode.Node node) {
            if (autoConnectOutput == null) return;

            // Find input port of same type
            ZTool.XNode.NodePort inputPort = node.Ports.FirstOrDefault(x => x.IsInput && x.ValueType == autoConnectOutput.ValueType);
