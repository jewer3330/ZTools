        ...
        private void OnGUI() {
            ...
            if (input == null || input.Trim() == "") {
                if (GUILayout.Button("Revert to default") || (e.isKey && e.keyCode == KeyCode.Return)) {
                    ...
                    NodeEditor.GetEditor((ZTool.XNode.Node)target, NodeEditorWindow.current).OnRename();
                    if (!string.IsNullOrEmpty(AssetDatabase.GetAssetPath(target))) {
                        AssetDatabase.SetMainObject((target as ZTool.XNode.Node).graph, AssetDatabase.GetAssetPath(target));
