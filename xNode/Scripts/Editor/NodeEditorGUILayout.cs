...
namespace XNodeEditor {
    ...
    public static class NodeEditorGUILayout {
        ...
        public static void PropertyField(SerializedProperty property, GUIContent label, bool includeChildren = true, params GUILayoutOption[] options) {
            ...
            ZTool.XNode.Node node = property.serializedObject.targetObject as ZTool.XNode.Node;
            ZTool.XNode.NodePort port = node.GetPort(property.name);
            PropertyField(property, label, port, includeChildren);
        }

        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, ZTool.XNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options) {
            PropertyField(property, null, port, includeChildren, options);
        }

        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, GUIContent label, ZTool.XNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options) {
            if (property == null) throw new NullReferenceException();
            ...
            if (port == null) EditorGUILayout.PropertyField(property, label, includeChildren, GUILayout.MinWidth(30));
                ...
                if (port.direction == ZTool.XNode.NodePort.IO.Input) {
                    // Get data from [Input] attribute
                    ZTool.XNode.Node.ShowBackingValue showBacking = ZTool.XNode.Node.ShowBackingValue.Unconnected;
                    ZTool.XNode.Node.InputAttribute inputAttribute;
                    ...
                    bool usePropertyAttributes = dynamicPortList ||
                        showBacking == ZTool.XNode.Node.ShowBackingValue.Never ||
                        (showBacking == ZTool.XNode.Node.ShowBackingValue.Unconnected && port.IsConnected);
                    ...
                    if (dynamicPortList) {
                        ...
                        ZTool.XNode.Node.ConnectionType connectionType = inputAttribute != null ? inputAttribute.connectionType : ZTool.XNode.Node.ConnectionType.Multiple;
                        DynamicPortList(property.name, type, property.serializedObject, port.direction, connectionType);
                        return;
                    }
                    switch (showBacking) {
                        case ZTool.XNode.Node.ShowBackingValue.Unconnected:
                            ...
                            break;
                        case ZTool.XNode.Node.ShowBackingValue.Never:
                            // Display a label
                            EditorGUILayout.LabelField(label != null ? label : new GUIContent(property.displayName, tooltip));
                            break;
                        case ZTool.XNode.Node.ShowBackingValue.Always:
                ...
                } else if (port.direction == ZTool.XNode.NodePort.IO.Output) {
                    // Get data from [Output] attribute
                    ZTool.XNode.Node.ShowBackingValue showBacking = ZTool.XNode.Node.ShowBackingValue.Unconnected;
                    ZTool.XNode.Node.OutputAttribute outputAttribute;
                    bool dynamicPortList = false;
                    ...
                    if (dynamicPortList) {
                        Type type = GetType(property);
                        ZTool.XNode.Node.ConnectionType connectionType = outputAttribute != null ? outputAttribute.connectionType : ZTool.XNode.Node.ConnectionType.Multiple;
                        DynamicPortList(property.name, type, property.serializedObject, port.direction, connectionType);
                        return;
                    }
                    switch (showBacking) {
        ...
        public static void PortField(ZTool.XNode.NodePort port, params GUILayoutOption[] options) {
            PortField(null, port, options);
        }

        /// <summary> Make a simple port field. </summary>
        public static void PortField(GUIContent label, ZTool.XNode.NodePort port, params GUILayoutOption[] options) {
            if (port == null) return;
        ...
        /// <summary> Make a simple port field. </summary>
        public static void PortField(Vector2 position, ZTool.XNode.NodePort port) {
            if (port == null) return;
        ...
        public static void AddPortField(ZTool.XNode.NodePort port) {
            if (port == null) return;
            Rect rect = new Rect();
        ...
        public static void PortPair(ZTool.XNode.NodePort input, ZTool.XNode.NodePort output) {
        ...
        public static bool IsInstancePortListPort(ZTool.XNode.NodePort port) {
            return IsDynamicPortListPort(port);
        }

        [Obsolete("Use DynamicPortList instead")]
        public static void InstancePortList(string fieldName, Type type, SerializedObject serializedObject, ZTool.XNode.NodePort.IO io, ZTool.XNode.Node.ConnectionType connectionType = ZTool.XNode.Node.ConnectionType.Multiple, ZTool.XNode.Node.TypeConstraint typeConstraint = ZTool.XNode.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null) {
        ...
        public static bool IsDynamicPortListPort(ZTool.XNode.NodePort port) {
        ...
        public static void DynamicPortList(string fieldName, Type type, SerializedObject serializedObject, ZTool.XNode.NodePort.IO io, ZTool.XNode.Node.ConnectionType connectionType = ZTool.XNode.Node.ConnectionType.Multiple, ZTool.XNode.Node.TypeConstraint typeConstraint = ZTool.XNode.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null) {
            ZTool.XNode.Node node = serializedObject.targetObject as ZTool.XNode.Node;
            ...
            var indexedPorts = node.DynamicPorts.Select(x => {
                ...
                return new { index = -1, port = (ZTool.XNode.NodePort)null };
            }).Where(x => x.port != null);
            List<ZTool.XNode.NodePort> dynamicPorts = indexedPorts.OrderBy(x => x.index).Select(x => x.port).ToList();
        ...
        private static ReorderableList CreateReorderableList(string fieldName, List<ZTool.XNode.NodePort> dynamicPorts, SerializedProperty arrayData, Type type, SerializedObject serializedObject, ZTool.XNode.NodePort.IO io, ZTool.XNode.Node.ConnectionType connectionType, ZTool.XNode.Node.TypeConstraint typeConstraint, Action<ReorderableList> onCreation) {
            ...
            list.drawElementCallback =
                (Rect rect, int index, bool isActive, bool isFocused) => {
                    ZTool.XNode.NodePort port = node.GetPort(fieldName + " " + index);
                ...
                (ReorderableList rl) => {
                    ...
                    if (rl.index > reorderableListIndex) {
                        for (int i = reorderableListIndex; i < rl.index; ++i) {
                            ZTool.XNode.NodePort port = node.GetPort(fieldName + " " + i);
                            ZTool.XNode.NodePort nextPort = node.GetPort(fieldName + " " + (i + 1));
            ...
            list.onAddCallback =
                (ReorderableList rl) => {
                    ...
                    if (io == ZTool.XNode.NodePort.IO.Output) node.AddDynamicOutput(type, connectionType, ZTool.XNode.Node.TypeConstraint.None, newName);
                    ...
                    if (dynamicPorts[index] == null) {
                        ...
                        for (int k = index + 1; k < dynamicPorts.Count(); k++) {
                            for (int j = 0; j < dynamicPorts[k].ConnectionCount; j++) {
                                ZTool.XNode.NodePort other = dynamicPorts[k].GetConnection(j);
            ...
            if (hasArrayData) {
                ...
                while (dynamicPortCount < arrayData.arraySize) {
                    ...
                    if (io == ZTool.XNode.NodePort.IO.Output) node.AddDynamicOutput(type, connectionType, typeConstraint, newName);
                    else node.AddDynamicInput(type, connectionType, typeConstraint, newName);
                    EditorUtility.SetDirty(node);
                    dynamicPortCount++;
                }
