...
namespace XNodeEditor {
    ...
    [CustomNodeGraphEditor(typeof(ZTool.XNode.NodeGraph))]
    public class NodeGraphEditor : XNodeEditor.Internal.NodeEditorBase<NodeGraphEditor, NodeGraphEditor.CustomNodeGraphEditorAttribute, ZTool.XNode.NodeGraph> {
        ...
        public virtual string GetNodeMenuName(Type type) {
            ...
            ZTool.XNode.Node.CreateNodeMenuAttribute attrib;
        ...
        public virtual void AddContextMenuItems(GenericMenu menu, Type compatibleType = null, ZTool.XNode.NodePort.IO direction = ZTool.XNode.NodePort.IO.Input) {
            ...
            for (int i = 0; i < nodeTypes.Length; i++) {
                ...
                if (disallowed) menu.AddItem(new GUIContent(path), false, null);
                    ...
                    ZTool.XNode.Node node = CreateNode(type, pos);
        ...
        public virtual Gradient GetNoodleGradient(ZTool.XNode.NodePort output, ZTool.XNode.NodePort input) {
        ...
        /// <param name="output"> The output this noodle comes from. Never null. </param>
        /// <param name="input"> The output this noodle comes from. Can be null if we are dragging the noodle. </param>
        public virtual float GetNoodleThickness(ZTool.XNode.NodePort output, ZTool.XNode.NodePort input) {
            return NodeEditorPreferences.GetSettings().noodleThickness;
        }

        public virtual NoodlePath GetNoodlePath(ZTool.XNode.NodePort output, ZTool.XNode.NodePort input) {
            return NodeEditorPreferences.GetSettings().noodlePath;
        }

        public virtual NoodleStroke GetNoodleStroke(ZTool.XNode.NodePort output, ZTool.XNode.NodePort input) {
            return NodeEditorPreferences.GetSettings().noodleStroke;
        }

        /// <summary> Returned color is used to color ports </summary>
        public virtual Color GetPortColor(ZTool.XNode.NodePort port) {
        ...
        /// <returns></returns>
        public virtual GUIStyle GetPortStyle(ZTool.XNode.NodePort port) {
        ...
        public virtual Color GetPortBackgroundColor(ZTool.XNode.NodePort port) {
            return Color.gray;
        }
        ...
        public virtual string GetPortTooltip(ZTool.XNode.NodePort port) {
        ...
        public virtual ZTool.XNode.Node CreateNode(Type type, Vector2 position) {
            Undo.RecordObject(target, "Create Node");
            ZTool.XNode.Node node = target.AddNode(type);
        ...
        public virtual ZTool.XNode.Node CopyNode(ZTool.XNode.Node original) {
            Undo.RecordObject(target, "Duplicate Node");
            ZTool.XNode.Node node = target.CopyNode(original);
        ...
        public virtual bool CanRemove(ZTool.XNode.Node node) {
            // Check graph attributes to see if this node is required
            Type graphType = target.GetType();
            ZTool.XNode.NodeGraph.RequireNodeAttribute[] attribs = Array.ConvertAll(
                graphType.GetCustomAttributes(typeof(ZTool.XNode.NodeGraph.RequireNodeAttribute), true), x => x as ZTool.XNode.NodeGraph.RequireNodeAttribute);
        ...
        public virtual void RemoveNode(ZTool.XNode.Node node) {
        ...
        XNodeEditor.Internal.NodeEditorBase<NodeGraphEditor, NodeGraphEditor.CustomNodeGraphEditorAttribute, ZTool.XNode.NodeGraph>.INodeEditorAttrib {
